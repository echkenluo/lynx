...
var _ = Describe("SecurityPolicy", func() {
	...
	Context("environment with virtual machines provide public http service [Feature:TCP] [Feature:ICMP]", func() {
		...
		When("limits tcp packets between components", func() {
			...
			BeforeEach(func() {
				nginxPolicy = newPolicy("nginx-policy", tier1, 50, []string{}, nginxGroup.Name)
				...
				serverPolicy = newPolicy("server-policy", tier1, 50, []string{}, serverGroup.Name)
				addIngressRule(serverPolicy, "TCP", serverPort, nginxGroup.Name)
				addEngressRule(serverPolicy, "TCP", dbPort, dbGroup.Name)

				dbPolicy = newPolicy("db-policy", tier1, 50, []string{}, dbGroup.Name)
		...
		When("limits icmp packets between components", func() {
			...
			BeforeEach(func() {
				icmpDropPolicy = newPolicy("icmp-drop-policy", tier1, 50, []string{}, serverGroup.Name, dbGroup.Name)
				addIngressRule(icmpDropPolicy, "TCP", 0) // allow all tcp packets

				icmpAllowPolicy = newPolicy("icmp-allow-policy", tier1, 50, []string{}, nginxGroup.Name)
	...
	Context("vm isolation", func() {
		...
		When("Isolate vm", func() {
			...
			BeforeEach(func() {
				isolationPolicy = newPolicy("isolation-policy", tier0, 50, []string{db01.Name})
		...
		When("Forensic vm", func() {
			...
			BeforeEach(func() {
				forensicPolicy = newPolicy("forensic-policy", tier0, 100, []string{db01.Name})
	...
	Context("environment with virtual machines provide internal udp service [Feature:UDP] [Feature:IPBlocks]", func() {
		...
		When("limits udp packets by ipBlocks between server and client", func() {
			...
			BeforeEach(func() {
				ntpProductionPolicy = newPolicy("ntp-production-policy", tier1, 50, []string{}, ntpProduction.Name)
				addIngressRule(ntpProductionPolicy, "UDP", ntpPort, productionCidr)

				ntpDevelopmentPolicy = newPolicy("ntp-development-policy", tier1, 50, []string{}, ntpDevelopment.Name)
...
func newPolicy(name, tier string, priority int32, endpoints []string, appliedGroup ...string) *securityv1alpha1.SecurityPolicy {
